...
const CELL_SIZE = 100;
const COLS = Math.floor(STYLECONFIG.WIDTH / CELL_SIZE);
const ROWS = Math.floor(STYLECONFIG.HEIGHT / CELL_SIZE);
 ...
 */
function bfs(startCol, startRow, blocked) {
  const VISITED = new Map();
  VISITED.set(cellKey(startCol, startRow), { distance: 0, parent: null });
  const QUEUE = [[startCol, startRow]];

  while (QUEUE.length > 0) {
    const [col, row] = QUEUE.shift();
    const CURRENT = VISITED.get(cellKey(col, row));
    ...
    neighbors(col, row).forEach(([nc, nr]) => {
      ...
      VISITED.set(KEY, { distance: CURRENT.distance + 1, parent: cellKey(col, row) });
      QUEUE.push([nc, nr]);
    });
  };
 ...
 */
function pickFarthestCell(visited, startKey) {
  let best = null;
 ...
 */
function reconstructPath(visited, targetKey) {
  const PATH = [];
  let key = targetKey;
  while (key) {
    PATH.unshift(key);
...
export function generateLevel(levelConfig) {
  const START = pixelToCell(levelConfig.human.x, levelConfig.human.y);
  const START_KEY = cellKey(START.col, START.row);
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    for (let i = 0; i < OBSTACLE_COUNT; i++) {
      ...
      ENVIRONMENT.push({ type: TYPE, x, y });
      OCCUPIED.add(KEY);
    ...
    const VISITED = bfs(START.col, START.row, BLOCKED);
    const HIDE_KEY = pickFarthestCell(VISITED, START_KEY);

    if (!HIDE_KEY || VISITED.get(HIDE_KEY).distance < MIN_HIDE_DISTANCE) {
      continue;
    };

    const PATH = reconstructPath(VISITED, HIDE_KEY);
    ...
    return { environment: ENVIRONMENT, solution: { waypoints: WAYPOINTS, explanations: TEXTS } };
  };

  return {
    environment: [],
    solution: { waypoints: [{ x: levelConfig.human.x, y: levelConfig.human.y }], explanations: [EXPLANATIONS.FIRST] },
  };
}
